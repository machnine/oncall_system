...
from .models import TimeBlock, TimeEntry, Detail, Donor, Recipient, LabTask, Assignment, DayType, ASSIGNMENT_TYPE_CONFIG
...
class TimeBlockForm(forms.ModelForm):
    ...
    entity_id = forms.CharField(
        max_length=50,
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'entity_id', 'placeholder': 'Enter ID or select from shortcuts'})
    )
    assignment_notes = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 2, 'placeholder': 'Optional notes about this assignment'})
    )
    
    # Hidden field for assignments JSON data
    assignments_data = forms.CharField(
        required=False,
    ...
    def get_day_type(self, date):
        ...
        try:
            day_type, created = DayType.objects.get_or_create(
                name=day_type_name,
                defaults={
                    'color': self._get_default_color(day_type_name)
                }
            )
            return day_type
        except Exception:
                ...
                try:
                    ...
                    for assignment_data in assignments:
                        ...
                        if assignment_type and entity_id:
                            ...
                            entity = self._get_or_create_entity(assignment_type, entity_id)
    ...
    def _get_or_create_entity(self, entity_type, entity_id):
        ...
        try:
            if entity_type == 'donor':
                ...
                entity, created = Recipient.objects.get_or_create(recipient_id=entity_id)
                return entity
